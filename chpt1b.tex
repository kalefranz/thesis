...
\glossary{name={$\psi_C$}, description={conduction band wavefunction}, sort=123}
\glossary{name={$\hslash$}, description={Planck's constant}, sort=h}
...
\glossary{name={$V_C$}, description={conduction band potential \slunit{Volts}}, sort=v}
...
\end{equation}
\glossary{name={$E_{{field}}$}, description={applied electric field}, sort=e}
%and
%\begin{equation}
%\begin{split}
...
\glossary{name={$z_it{second}$}, description={second discrete solve point after $z=0$}, sort=z}
...
\glossary{name={$\delta\EE_{{Varsh}}$}, description={Varshney correction to the bandgap energy for $T\neq0$~K}, sort=104}
...
\end{equation}
The bandgaps given in Eq.~\eqref{chpt1eqn:bandgap_eqns} neglect temperature effects, which can be included by adding the $\delta\!\EE_{Varsh}$ term.  A comparison of the effect of strain on In\sub{x}Ga\sub{1-x}As and Al\sub{1-x}In\sub{x}As bandgaps is shown in Fig.~\ref{chpt1:strain_effect}.
...
%\begin{equation}
%n_j=\frac{m k_{B} T}{\pi \hslash^2} \ln \left[1+ e^{\frac{\EE-\EE_{F}(T)}{k_{B} T}}\right]
%\end{equation}
...
%We can now separate the two contributions to the optical transition rate, those of stimulated emission $W_{stim}$ and spontaneous emission $W_{spon}$
%\begin{subequations}
%\begin{gather}
%W_{stim}=\frac{q^2 n_{eff} \: \omega_0^3}{3 \pi \hslash \epsilon_0 c_0^3} z_{u\ell}^2 \mathcal{N}_{ph}\\
%W_{spon}=\frac{q^2 n_{eff} \: \omega_0^3}{3 \pi \hslash \epsilon_0 c_0^3} z_{u\ell}^2
%\end{gather}
%\end{subequations}
...
%
%
%Material gain $\gamma~[\frac{1}{{length}}]$ is defined such that light intensity $\mathcal{I}$ after some distance $L$ is given by
%\begin{equation}
%\mathcal{I}(L) = \mathcal{I}(0) e^{\gamma L}
%\end{equation}
%and
%\begin{equation}
%\gamma = \begin{subarray}{c}{gain region}\\{overlap with}\\{optical mode}\end{subarray} \times \begin{subarray}{c}{``strength'' of the}\\{gain material}\end{subarray}
%\times \begin{subarray}{c}{population}\\{inversion}\end{subarray} = \Gamma \sigma_0 (N_u-N_\ell)
%\end{equation}
%where $(N_u-N_\ell)~[\frac{1}{{volume}}]$ represents the population inversion density between the upper $u$ and lower $\ell$ states of the optical transition. The transition (gain) cross-section at peak gain $\sigma_0 \left[{area}\right]$ as previously derived for the mode photon energy $\EE_{ph}$ is
%\begin{equation}
%\sigma_0 = \frac{4 \pi q^2}{h c_0 \epsilon_0 n_{eff}} \frac{\EE_{ph}}{\delta\!\EE_{u\ell}} z_{u\ell}^2
%\end{equation}
...
%\gamma = \alpha_{total}
%\end{equation}
%with total optical loss $\alpha_{total}~[\frac{1}{{length}}]$ being the sum of mirror loss $\alpha_m$ and waveguide loss $\alpha_w$.  Assuming each active period has the same transition cross-section and lifetime profile, applying Eq.~\eqref{chpt1eqn:gamma_sum} to Eq.~\eqref{chpt1eqn:gain_clamp} yields
%\begin{equation}
%\alpha_m + \alpha_w = N_p \Gamma \sigma_0 (N_{u}-N_{\ell})_{th}
%\end{equation}
%where $N_p$ results from summing over all active periods and the population density $(N_{u}-N_{\ell})_{th}$ is at threshold.  For a single QC period,
%\begin{equation}
%(N_{u,i}-N_{\ell,i})_{th} = \begin{subarray}{c}{pumping}\\{rate}\end{subarray} \times \begin{subarray}{c}{effective upper}\\{state lifetime}\end{subarray}
%= \frac{J_{th}}{q d_{ap,i}} \tau_{eff,i}
%\end{equation}
%where $\tau_{eff}$ is the effective upper state lifetime.  For the case of a two level system at threshold (neglecting stimulated emission), $\tau_{eff}=\tau_u \left(1-\frac{\tau_\ell}{\tau_{u\ell}}\right)$.  Thus,
%\begin{equation}
%J_{th} = q \frac{\alpha_{total}}{ \Gamma \frac{N_p}{L_{ac}} \sigma_0 \tau_{eff}} = \frac{h c_0 \epsilon_0 n_{eff}}{4 \pi q} \frac{\alpha_{total}}{ \Gamma \frac{N_p}{L_{ac}} \frac{\EE_{u\ell}}{\delta\!\EE_{u\ell}} z_{u\ell}^2 \tau_{eff} } {~.}
%\end{equation}
...
\end{equation}
Another common way to represent gain is as as modal gain $\gamma$ \slunit{length} defined such that light intensity $\mathcal{I}$ after some distance $L$ is given by
\begin{equation}
...
%\begin{equation}
%\frac{d P}{d\!I} = \frac{\EE_{ph}}{q} \frac{\alpha_m}{\alpha_m+\alpha_w}   \sum_{i=1}^{N_p} \eta_{inj,i} \frac{\tau_{eff,i}}{\left(\tau_{eff,i}+\tau_{\ell,i}\right)} \Gamma_{m,i} \text{~.}
%\end{equation}
%If all QC periods are the same,
%\begin{equation}
%\frac{d P}{d\!I} = N_p \frac{\EE_{ph}}{q} \frac{\alpha_m}{\alpha_m+\alpha_w}   \frac{\tau_{eff}}{\left(\tau_{eff}+\tau_{\ell}\right)} \eta_{inj} \Gamma_{m} \text{~.}
%\end{equation}
...
\end{equation}
This transport velocity can be used as a fundamental component of current density $J$, which also multiplies the electron density participating in transport $N_{trans}$ \dlunit{1}{volume} \cite{Sze}.
\begin{equation}
...
%\begin{subequations}
%\begin{gather}
%\sigma = q \mu N_{trans} = q N_{trans} \frac{v_{trans}}{E_{field}}\\
%R = \rho \frac{\ell}{A} = \frac{1}{\sigma} \frac{L_p}{A}\\
%R = \frac{E_{field}}{q N_{trans} v_{trans}} \frac{L_p}{A} = \frac{E_{field}}{q N_{trans}} \frac{\tau_{trans}}{A}\\
%E_{field} = \frac{V_{appl}}{L_p}\\
%R = \frac{1}{q A} \frac{V_{appl}}{n_{trans}}  \tau_{trans}
%\end{gather}
%\end{subequations}
